...
/*    public static void main(String[] args) {  
     ...
     * 根据指定日期得到addDay的日期
     */
    public static String addDayDateFormat(String date, int addday) {
    	int year = Integer.parseInt(date.substring(0, 4));
    	int month = Integer.parseInt(date.substring(5, 7)) - 1;
    	int day = Integer.parseInt(date.substring(8));
    	Calendar cal = Calendar.getInstance();
    	...
    	cal.set(Calendar.DAY_OF_MONTH, day);
    	cal.add(Calendar.DATE, addday);
    	return new SimpleDateFormat(DateConstants.DATE_Format).format(cal.getTime());
    }
     ...
     * @throws Exception
     */
    public static String convertFStringToString(String strDate) {
        return (strDate == null ? "" : strDate).replaceAll("-", "/");
    }
     ...
     * @throws Exception
     */
    public static Date convertStringToDateTime9(String strDate,String pattern) {
     ...
     * @return
     */
    public static String checkDateFormate3(String dateStr) {
    	String resultStr = "";
        try {
     ...
     * @throws ParseException
     */
    public static long getCompareSeconds(String first, String second) {
        Date dt;
        Date dts;
        SimpleDateFormat ft = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        try {
    ...
    public static String getDateByDefaultDate(String dateDefault) {
        // 解决服务器日期时区转化问题
        if (dateDefault.contains("GMT+08:00")) {
            dateDefault = dateDefault.replace("GMT+08:00", "CST");
        }
        if (dateDefault.contains("GMT 08:00")) {
            dateDefault = dateDefault.replace("GMT 08:00", "CST");
        }
        Date date = null;
        if (!Tools.isblank(dateDefault) && dateDefault.length() == 10) {
            date = DateUtil.getDate(dateDefault, DateConstants.DATE_PATTERN);
            return dateDefault;
        } else {
    ...
    public static Date getLastSundayOfWeek(Date date){
        Calendar cal = Calendar.getInstance();  
        cal.setTime(date);  
        //n为推迟的周数，1本周，0向前推迟一周，2下周，依次类推
        int n = 0 ;
        cal.add(Calendar.DATE, n*7);
        //想周几，这里就传周几Calendar.MONDAY（TUESDAY...）
     ...
     * @throws ParseException 
     */
    public static Date getFixedDateOfSunnday(Date date) {
//上周周日
        Calendar cal = Calendar.getInstance();  
        cal.setTime(date);  
        //n为推迟的周数，1本周，0向前推迟一周，2下周，依次类推
...
//本月第几周        
        Calendar numweekcal = Calendar.getInstance();  
        numweekcal.setTime(lastSunday);
        //中国周一是一周的第一天
        numweekcal.setFirstDayOfWeek(Calendar.MONDAY);
        //第几周
        int week = numweekcal.get(Calendar.WEEK_OF_MONTH);
//往前推3个自然月所在周日        
        Calendar threecalendar = Calendar.getInstance();//日历对象
        threecalendar.setTime(date);//设置当前日期
        int flag = -3;//月份减一为-1，加一为1
        threecalendar.add(Calendar.MONTH, flag);
        
        Calendar threecalendarsunday = Calendar.getInstance();  
        threecalendarsunday.setTime(threecalendar.getTime());  
        //n为推迟的周数，1本周，0向前推迟一周，2下周，依次类推
        threecalendarsunday.add(Calendar.DATE, n*7);
        threecalendarsunday.set(Calendar.DAY_OF_WEEK,Calendar.SUNDAY);
        //前推3个自然月上周周日日期
        return threecalendarsunday.getTime();
    }
    ...
    /**
     * 根据生日计算年龄
     * bug 8603  患者出生日期入参为空时 直接返回0 zhanghaibo
     * @param birthDay
     * @return
     * @throws Exception
     */
    ...
    //获取某天所在星期的周一
    public static String getfirstDayOfWeek (String oneday) {
        String firstDay = "";
        try {
            SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd"); // 设置时间格式
            Calendar cal = Calendar.getInstance();
            Date time = sdf.parse(oneday);
            cal.setTime(time);
    ...
    //获取某天所在星期的周日
    public static String getlastDayOfWeek (String oneday) {
        String lasttDay = "";
        try {
            SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd"); // 设置时间格式
            Calendar cal = Calendar.getInstance();
            ...
            cal.add(Calendar.DATE, 6);
            lasttDay = sdf.format(cal.getTime());
        } catch (ParseException e) {
            e.printStackTrace();
        }
        return lasttDay;
    ...
    /**
     * 将yyyy-MM-dd转化为yyyy年MM月dd日格式
     * @param datepattern
     * @return
     */
    public static String getDateNormal(String datepattern) {
    	String str = "";
    	if (!Tools.isblank(datepattern)) {
    		Date date = getDate(datepattern, DateConstants.DATE_PATTERN);
    		SimpleDateFormat sdf = new SimpleDateFormat(DateConstants.DATE_NORMAL);
    		str = sdf.format(date);
		}
		return str;
    }
     ...
     * @return
     */
    public static List<String> getAllDates(String startTime, String endTime) {
     ...
     * @return
     */
    public static Date getSimpleDate(Date date, String dateFormat) {
        return getDate(formatDate(date, dateFormat), dateFormat);
    }
    
    /**
     * 根据时间获取上个季度的第一天
     */
    public static Date getFirstDayOfLastSeason(Date date){
        Calendar cal = Calendar.getInstance();
        cal.setTime(date);
     ...
     * 根据时间获取上个季度的最后一天
     */
    public static Date getLastDayOfLastSeason(Date date){
        Calendar cal = Calendar.getInstance();
        cal.setTime(date);
